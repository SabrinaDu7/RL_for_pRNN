...
def run_benchmark(cfg, n_updates: int, include_plot: bool, include_analysis: bool) -> dict:
    import torch
    ...
    if len(ts) and "loss" in ts:
        metrics["prnn_loss"] = [float(x) for x in ts["loss"].tolist()]
    ...
    if include_analysis:
        from curious_george.training.loop import run_behavior_analysis, run_spatial_analysis
...
def main():
    ...
    if args.profile:
        ...
        prof.enable()
        result = run_benchmark(cfg, args.updates, args.include_plot, args.include_analysis)
        prof.disable()
