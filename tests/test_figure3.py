...
from scripts.figure3 import _names_cache_key, fetch_run_traces_by_names_cached
...
def test_fetch_by_names_concatenates_runs():
    ...
    with patch("scripts.figure3.fetch_run_traces", side_effect=mock_fetch):
...
def test_fetch_by_names_uses_cache(tmp_path):
    from scripts.figure4 import save_df
    from scripts.figure3 import CACHE_DIR
    ...
    try:
        with patch("scripts.figure3.fetch_run_traces") as mock_fetch:
            result = fetch_run_traces_by_names_cached(
                "entity", "proj", "metric", "_step",
